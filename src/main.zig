...
pub fn main() !void {
    ...
    registry.register(
        "skill_manage",
        "Manage skills (create, update, delete). Skills are procedural memory. Actions: create (full SKILL.md), patch (old_string/new_string), edit (full rewrite), delete, write_file, remove_file. After difficult tasks, offer to save as a skill. Confirm before creating/deleting.",
        std.json.parseFromSliceLeaky(std.json.Value, allocator,
            \\{"type":"object","properties":{"action":{"type":"string","enum":["create","patch","edit","delete","write_file","remove_file"],"description":"Action to perform"},"name":{"type":"string","description":"Skill name (lowercase, hyphens)"},"content":{"type":"string","description":"Full SKILL.md for create/edit"},"category":{"type":"string","description":"Category subdirectory for create"},"old_string":{"type":"string","description":"Text to find (patch)"},"new_string":{"type":"string","description":"Replacement text (patch)"},"replace_all":{"type":"boolean","description":"Replace all occurrences (patch)"},"file_path":{"type":"string","description":"Supporting file path"},"file_content":{"type":"string","description":"Content for write_file"}},"required":["action","name"]}
        , .{}) catch .null,
        skill_manager.SkillManager.dummyHandler,
        "skills",
    );
